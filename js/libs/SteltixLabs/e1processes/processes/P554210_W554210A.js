	...
	var Process = function(){
		...
		self.init = function() {
	        ...
	        _.getForm("appstack",reqObj).then(function(data){
	            if (data.hasOwnProperty("fs_P554210_W554210E")) {
	                var rowArr = data.fs_P554210_W554210E.data.gridData.rowset;
					var errObj = data.fs_P554210_W554210E.errors;
		...
		self.selectRow = function(inputRow) {
			...
			_.getForm("appstack",reqObj).then(function(data){
			    if (data.hasOwnProperty("fs_P554210_W554210A")) {
					...
					var fieldObj = data.fs_P554210_W554210A.data;
					if (errObj.length > 0) {
			    	    _.getErrorMsgs(errObj);
			    	    _.returnFromError();
						...
						//globals.htmlInputs = data.fs_P554210_W554210A.data;
						//globals.titleList = globals.htmlInputs.gridData.titles;
						
			            var rowsetArr = data.fs_P554210_W554210A.data.gridData.rowset;
		...
		self.addRow = function(inputRow) {
			// open add form
			var reqObj = _.buildAppstackJSON({form: "W554210E"},"87");
			_.getForm("appstack",reqObj).then(function(data){
			    if (data.hasOwnProperty("fs_P554210_W554210A")) {
					var errObj = data.fs_P554210_W554210A.errors;
					var fieldObj = data.fs_P554210_W554210A.data;
					if (errObj.length > 0) {
						...
						_.getForm("appstack",reqObj).then(function(data){
							if (data.hasOwnProperty("fs_P554210_W554210A")) { 
								var errObj = data.fs_P554210_W554210A.errors;
                            	var warningObj = data.fs_P554210_W554210A.warnings;
		...
		self.updateForm = function (rowToUpdate, inputRow) {
			...
			*/
			...
			_.getForm("appstack",reqObj).then(function(data){
				if (data.hasOwnProperty("fs_P5542101_W5542101D")) { 
					var errObj = data.fs_P5542101_W5542101D.errors;
					if (errObj.length > 0) {
			    	    _.getErrorMsgs(errObj);
			    	    _.returnFromError();
					} 
					var warningObj = data.fs_P5542101_W5542101D.warnings;
