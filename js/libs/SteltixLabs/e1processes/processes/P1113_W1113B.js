define(['aisclient'],function(_){
    var Process = function(){
                ...
                _.getForm("appstack",reqObj).then(function(data){
                    if (data.hasOwnProperty("fs_P1113_W1113B")) { // is form being returned? if not, lets try add.
                        ...
                        if (errObj.length > 0) {
                            ...
                            self.processRows(inputRow, rowArr, data.fs_P1113_W1113B.data);
                        } else {
        ...
        self.processRows = function(inputRow, dataRows, dataObj) {
            ...
            if (inputRow.EXTRACT) {
                $.each(dataObj.gridData.rowset,function(key,object) {
                        if (dataObj.gridData.rowset[key].hasOwnProperty("MOExist")) 
                            delete dataObj.gridData.rowset[key].MOExist;
                        if (dataObj.gridData.rowset[key].hasOwnProperty("rowIndex")) 
                            delete dataObj.gridData.rowset[key].rowIndex; 
                });

                _.appendTable(dataObj, 0, true);
                ...
                if (numberUpdates || effDateUpdate)
                {
                    _.getForm("appstack",reqObj).then(function(data){
                        if (data === 'no_response') {
                            ...
                            if (data.hasOwnProperty("fs_P1113_W1113B")) {
                                var errObj = data.fs_P1113_W1113B.errors;
                                var warningObj = data.fs_P1113_W1113B.warnings;
                                if (errObj.length > 0) {
                                    _.getErrorMsgs(errObj);
                                    _.returnFromError();
