...
define(['aisclient'], function(_){
    var Process = function(){
        ...
        self.getAddForm = function(inputRow) {
        	...
        	_.getForm("appstack",reqObj).then(function(data){
        		if (data.hasOwnProperty("fs_P1782_W1782H")) {
                    if (globals.htmlInputs.length === 0) {
                        globals.htmlInputs = data.fs_P1782_W1782H.data;
                    }
        	    ...
        	    if (data.hasOwnProperty("fs_P1782_W1782G")) {
        	    	...
        	    	if (rowsetArr.length > 0) {
        	    		for (var i = 0; i < rowsetArr.length; i++) {
    	                    if (rowsetArr[i].sInformationTypeDescription_28.value === "Supplier") {
        ...
        self.getEditForm = function(inputRow,rowToSelect) { // UPDATE
        	...
        	_.getForm("appstack",reqObj).then(function(data){
        		if (data.hasOwnProperty("fs_P1782_W1782H")) {
        			...
        			var errObj = data.fs_P1782_W1782H.errors;
        			if (errObj.length > 0) {
        			    _.getErrorMsgs(errObj);
        			    _.returnFromError();
        			} else {
