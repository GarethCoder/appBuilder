...
define(['aisclient'], function(_){
    var Process = function(){
        ...
        self.selectRow = function(inputRow) {
            ...
            var reqObj = _.buildAppstackJSON(optionsObj,"1.0","134");
            _.getForm("appstack",reqObj).then(function(data){
                if (data.hasOwnProperty("fs_P4101_W4101C")) {
                    var errObj = data.fs_P4101_W4101C.errors;
                    if (errObj.length > 0) {
                        _.getErrorMsgs(errObj);
                        _.returnFromError();
                        ...
                        // add the LITM to the data returned to ensure no blank column in extracted spreadsheet
                        data.fs_P4101_W4101C.data.z_LITM_13 = data.fs_P4101_W4101C.data.z_UITM_307
                        data.fs_P4101_W4101C.data.z_LITM_13.id = 13;
                        _.appendTable(data.fs_P4101_W4101C.data, self.reqFields.titles);
                    } else {
                    	if (globals.htmlInputs.length === 0) { // Will only be true if no data has ever been added to that key
    			    		globals.htmlInputs = data.fs_P4101_W4101C.data;
    			        }
